...
impl Status {
    /// Translates a raw `blink_status` value.
    #[must_use]
    pub fn from_raw(status: sys::blink_status) -> Self {
        match status {
            sys::BLINK_ERR_INVALID_ARGUMENT => Self::InvalidArgument,
            sys::BLINK_ERR_DEVICE_NOT_FOUND => Self::DeviceNotFound,
            sys::BLINK_ERR_OUT_OF_RANGE => Self::OutOfRange,
            sys::BLINK_ERR_UNSUPPORTED => Self::Unsupported,
            sys::BLINK_ERR_IO => Self::Io,
            sys::BLINK_ERR_OUT_OF_MEMORY => Self::OutOfMemory,
            sys::BLINK_ERR_BUFFER_TOO_SMALL => Self::BufferTooSmall,
            sys::BLINK_ERR_INTERNAL => Self::Internal,
            other => Self::Unknown(other),
        }
    }

    /// The raw `blink_status` value this status corresponds to.
    #[must_use]
    pub fn as_raw(self) -> sys::blink_status {
        match self {
            Self::InvalidArgument => sys::BLINK_ERR_INVALID_ARGUMENT,
            Self::DeviceNotFound => sys::BLINK_ERR_DEVICE_NOT_FOUND,
            Self::OutOfRange => sys::BLINK_ERR_OUT_OF_RANGE,
            Self::Unsupported => sys::BLINK_ERR_UNSUPPORTED,
            Self::Io => sys::BLINK_ERR_IO,
            Self::OutOfMemory => sys::BLINK_ERR_OUT_OF_MEMORY,
            Self::BufferTooSmall => sys::BLINK_ERR_BUFFER_TOO_SMALL,
            Self::Internal => sys::BLINK_ERR_INTERNAL,
            Self::Unknown(other) => other,
        }
    }

    /// The library's own name for this code, such as `"BLINK_ERR_IO"`.
    #[must_use]
    pub fn name(self) -> &'static str {
        // SAFETY: `blink_status_name` accepts any `int32_t` and returns a
        // non-NULL pointer to a string with static storage duration.
        let ptr = unsafe { sys::blink_status_name(self.as_raw()) };
        if ptr.is_null() {
...
impl Error {
    ...
    /// next `blink_*` call.
    pub(crate) fn from_raw(status: sys::blink_status) -> Self {
        Self {
...
pub(crate) fn check(status: sys::blink_status) -> Result<()> {
    if status == sys::BLINK_OK {
        Ok(())
    } else {
...
fn last_error_message() -> String {
    ...
    // `blink_*` call on this thread. It is copied before returning.
    let ptr = unsafe { sys::blink_last_error_message() };
    if ptr.is_null() {
        return String::new();
    }
...
pub fn clear_last_error() {
    ...
    unsafe { sys::blink_clear_last_error() }
}
