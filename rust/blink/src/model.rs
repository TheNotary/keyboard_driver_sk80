...
impl Model {
    ...
    /// The raw `blink_model` value.
    #[must_use]
    pub fn as_raw(self) -> sys::blink_model {
        match self {
            Self::Sk80 => sys::BLINK_MODEL_SK80,
            Self::Mk84 => sys::BLINK_MODEL_MK84,
            Self::Rk84 => sys::BLINK_MODEL_RK84,
        }
    }
    ...
    pub fn from_raw(raw: i32) -> Option<Self> {
        match u32::try_from(raw).ok()? {
            sys::BLINK_MODEL_SK80 => Some(Self::Sk80),
            sys::BLINK_MODEL_MK84 => Some(Self::Mk84),
            sys::BLINK_MODEL_RK84 => Some(Self::Rk84),
            _ => None,
        }
    }
    ...
    pub fn info(self) -> Result<KeyboardInfo> {
        ...
        // SAFETY: `raw` is a valid, writable `blink_keyboard_info`.
        check(unsafe { sys::blink_model_get_info(self.as_raw(), &mut raw) })?;
    ...
    pub fn key_count(self) -> Result<usize> {
        ...
        check(unsafe { sys::blink_model_key_count(self.as_raw(), &mut count) })?;
        Ok(count)
    }
    ...
    pub fn key_at(self, index: usize) -> Result<Key> {
        ...
        check(unsafe { sys::blink_model_key_at(self.as_raw(), index, &mut name, &mut id) })?;
        Ok(Key {
    ...
    pub fn key_id_for_name(self, name: &str) -> Result<u8> {
        ...
        check(unsafe { sys::blink_model_key_id_for_name(self.as_raw(), name.as_ptr(), &mut id) })?;
        Ok(id)
    }
}
...
impl KeyState {
    ...
    #[must_use]
    pub fn as_raw(self) -> sys::blink_key_value {
        match self {
            Self::Off => sys::BLINK_KEY_OFF,
            Self::On => sys::BLINK_KEY_ON,
        }
    }
}
...
impl KeyboardInfo {
    ...
    pub(crate) fn from_raw(raw: &sys::blink_keyboard_info) -> Result<Self> {
...
pub fn list_known_keyboards() -> Result<Vec<KeyboardInfo>> {
    ...
    list_with(|infos, capacity, count| unsafe {
        sys::blink_list_known_keyboards(infos, capacity, count)
    })
}
...
pub fn list_available_keyboards() -> Result<Vec<KeyboardInfo>> {
    // SAFETY: forwards a caller-owned buffer with an honest capacity.
    list_with(|infos, capacity, count| unsafe {
        sys::blink_list_available_keyboards(infos, capacity, count)
    })
}

/// Runs the ABI's probe-then-fill enumeration protocol.
///
/// A probing call with a zero capacity reports `BLINK_ERR_BUFFER_TOO_SMALL`
/// whenever there is anything at all to report -- a zero-capacity buffer cannot
/// hold it -- and only returns `BLINK_OK` when the result is empty. Either way
...
fn list_with(
    mut enumerate: impl FnMut(*mut sys::blink_keyboard_info, usize, *mut usize) -> sys::blink_status,
    ...
    if status != sys::BLINK_OK && status != sys::BLINK_ERR_BUFFER_TOO_SMALL {
    ...
    for _ in 0..MAX_ATTEMPTS {
        ...
        let mut raw = vec![sys::blink_keyboard_info::default(); count];
        let mut total = 0usize;
        let status = enumerate(raw.as_mut_ptr(), raw.len(), &mut total);

        if status == sys::BLINK_ERR_BUFFER_TOO_SMALL {
...
pub fn abi_version() -> (u16, u8, u8) {
    ...
    let raw = unsafe { sys::blink_abi_version() };
    (
...
/// (`blink_version_string`, `blink_status_name`, `blink_model_key_at`,
/// `blink_sk80_lcd_defaults`), so the data genuinely outlives any borrow.
