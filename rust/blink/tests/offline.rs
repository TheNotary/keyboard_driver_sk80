...
use blink::{Key, KeyState, Keyboard, Model, Rgb, Status};

#[test]
fn abi_is_compatible_with_the_loaded_library() {
    assert!(blink::abi_is_compatible());
    assert_eq!(blink::abi_version().0, 1);
}

#[test]
fn version_is_reported() {
    let version = blink::version();
...
fn known_keyboards_include_the_implemented_models() {
    let known = blink::list_known_keyboards().expect("enumeration should succeed");
...
fn available_keyboards_enumerates_without_hardware() {
    ...
    let available = blink::list_available_keyboards().expect("enumeration should succeed");
...
fn unimplemented_model_is_reported_as_unsupported() {
    ...
    assert_eq!(err.status().name(), "BLINK_ERR_UNSUPPORTED");
}
...
fn key_state_maps_to_the_c_abi_ordering() {
    ...
    assert_eq!(KeyState::Off.as_raw(), blink::sys::BLINK_KEY_OFF);
    assert_eq!(KeyState::On.as_raw(), blink::sys::BLINK_KEY_ON);
...
fn print_messages_rejects_a_buffer_that_is_too_short() {
    let err = blink::print_messages(&[0u8; 8], 2, 8).expect_err("8 bytes cannot hold 2 x 8");
    assert_eq!(err.status(), Status::InvalidArgument);

    // Overflowing the geometry must be caught rather than wrapping.
    let err = blink::print_messages(&[0u8; 8], usize::MAX, 2).expect_err("geometry overflows");
    assert_eq!(err.status(), Status::InvalidArgument);
}
...
fn errors_display_the_status_name() {
    ...
    assert!(
        err.to_string().starts_with("BLINK_ERR_UNSUPPORTED"),
        "{err}"
    );
}
