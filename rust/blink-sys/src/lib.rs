//! Raw FFI bindings to the [`blink`] C ABI for controlling RGB mechanical
//! keyboard LEDs and LCD displays.
//!
//! This crate is a mechanical translation of `blink.h` and applies no safety
//! rules of its own. Prefer the safe `blink` crate, which wraps everything here
...
//!    `BLINK_INCLUDE_DIR` must contain `blink/blink.h`.
...
//!   also links blink's own dependencies, because `blink_static` exposes hidapi
//!   as a public usage requirement.
//! * `bindgen` -- regenerate the declarations from `blink.h` at build time
...
//! * [`blink_last_error_message`] returns a thread-local pointer that is only
...
//! * Callbacks installed through [`blink_usb_io`] must not unwind.
//!
//! [`blink`]: https://github.com/TheNotary/keyboard_driver_sk80
...
pub const BLINK_ABI_VERSION: u32 =
    (BLINK_ABI_VERSION_MAJOR << 16) | (BLINK_ABI_VERSION_MINOR << 8) | BLINK_ABI_VERSION_PATCH;

impl Default for blink_usb_io {
...
pub fn abi_is_compatible() -> bool {
    ...
    let loaded = unsafe { blink_abi_version() };
    (loaded >> 16) == (BLINK_ABI_VERSION >> 16)
}
