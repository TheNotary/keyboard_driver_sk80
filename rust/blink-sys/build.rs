//! Locates the native `blink` library, emits the link directives for it, and
//! optionally regenerates the FFI bindings from `blink.h`.
//!
//! Discovery is tried in this order, and the first mode that succeeds wins:
//!
//! 1. `BLINK_LIB_DIR` + `BLINK_INCLUDE_DIR` -- an explicit install prefix.
//! 2. `pkg-config` -- the `blink.pc` installed by the CMake project.
...
struct Native {
    ...
    /// Static linkage drags in blink's own dependencies, because
    /// `blink_static` links hidapi as a PUBLIC usage requirement. The shared
...
fn main() {
    ...
    // no installed blink. Rustdoc never links, so skip discovery entirely.
    ...
    // DEP_BLINK_INCLUDE / DEP_BLINK_STATIC, courtesy of the `links` key.
    println!(
        "cargo:include={}",
        native
            .include_dirs
...
fn discover_vendored() -> Native {
    panic!(
        "could not locate the native `blink` library.\n\
         \n\
         Choose one of:\n\
         \x20 * set BLINK_LIB_DIR and BLINK_INCLUDE_DIR to an install prefix, e.g.\n\
         \x20     BLINK_LIB_DIR=/usr/local/lib BLINK_INCLUDE_DIR=/usr/local/include\n\
         \x20 * install blink so that pkg-config can find `blink.pc`\n\
...
fn discover_from_env() -> Option<Native> {
    let lib_dir = env::var_os("BLINK_LIB_DIR")?;
    let include_dir = env::var_os("BLINK_INCLUDE_DIR")?;
    ...
    assert!(
        lib_dir.is_dir(),
        "BLINK_LIB_DIR is not a directory: {}",
        lib_dir.display()
    );
    assert!(
        include_dir.join("blink").join("blink.h").is_file(),
        "BLINK_INCLUDE_DIR does not contain blink/blink.h: {}",
        include_dir.display()
    );
...
/// The `blink.pc` installed alongside the CMake package.
...
/// happens to name. Without this, reinstalling a different build of blink into
...
fn track_native_inputs(lib_dirs: &[PathBuf], include_dirs: &[PathBuf]) {
    ...
    for dir in include_dirs {
        let header = dir.join("blink").join("blink.h");
        if header.is_file() {
...
fn build_vendored() -> Native {
    ...
    assert!(
        ...
        "the `vendored` feature needs the CMake project at {}, but no CMakeLists.txt is there.\n\
         Set BLINK_LIB_DIR and BLINK_INCLUDE_DIR instead when building against a published crate.",
        repo.display()
    );

    let mut cfg = cmake::Config::new(&repo);
    cfg.define("BLINK_BUILD_TESTS", "OFF")
        .define("BLINK_BUILD_BENCHMARKS", "OFF")
        .define("BLINK_BUILD_DEMO", "OFF")
        .define("BLINK_BUILD_SHARED", "OFF")
        .define("BLINK_BUILD_STATIC", "ON")
        // Only the C ABI is bound, so the internal C++ headers are dead weight.
        .define("BLINK_INSTALL_CXX_HEADERS", "OFF")
...
/// `rust/blink-sys` -> `rust` -> repository root.
#[cfg(feature = "vendored")]
fn repository_root() -> PathBuf {
    ...
    manifest
        ...
        .expect("blink-sys should live two directories below the repository root")
        .to_path_buf()
}
...
/// On MSVC the shared library's import library is already called `blink.lib`, so
/// the static archive is renamed to avoid the collision. Everywhere else both
/// linkages share the `blink` stem and are told apart by the file extension.
fn library_name(static_link: bool) -> String {
    if let Ok(name) = env::var("BLINK_LIB_NAME") {
        return name;
    }
    let msvc = env::var("CARGO_CFG_TARGET_ENV").as_deref() == Ok("msvc");
    if static_link && msvc {
        "blink_static".to_string()
    } else {
        "blink".to_string()
    }
}

/// Emit the libraries `blink_static` exposes as PUBLIC usage requirements.
///
...
fn emit_transitive_deps() {
    ...
    // blink is C++ even though its interface is C.
    println!("cargo:rustc-link-lib=dylib=stdc++");
}
...
fn generate_bindings(include_dirs: &[PathBuf]) {
    #[cfg(feature = "bindgen")]
    {
        ...
        let mut builder = bindgen::Builder::default()
            .header("wrapper.h")
            .allowlist_function("blink_.*")
            .allowlist_type("blink_.*")
            .allowlist_var("BLINK_.*")
            ...
            .no_partialeq("blink_usb_io")
            .no_hash("blink_usb_io")
            .no_default("blink_usb_io")
        ...
        if cfg!(feature = "static") || cfg!(feature = "vendored") {
            builder = builder.clang_arg("-DBLINK_STATIC");
        }

        builder
            .generate()
            .expect("failed to generate bindings from blink.h")
