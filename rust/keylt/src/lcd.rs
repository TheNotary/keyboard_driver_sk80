...
pub fn sk80_lcd_defaults() -> Result<LcdDefaults> {
    ...
    check(unsafe { sys::keylt_sk80_lcd_defaults(&mut vid, &mut pid, &mut control, &mut data) })?;
    Ok(LcdDefaults {
        vid,
        pid,
...
///
/// ```no_run
/// let mut lcd = keylt::Lcd::open_sk80()?;
/// lcd.connect()?;
/// lcd.upload_image("splash.gif")?;
/// # Ok::<(), keylt::Error>(())
/// ```
pub struct Lcd {
    handle: NonNull<sys::keylt_lcd>,
...
impl Lcd {
    ...
    fn open_inner(
        ...
        let vtable = backend.as_mut().map(|state| {
            ...
            // handle does, and is only dropped after `keylt_lcd_close`.
            unsafe { BackendState::vtable(std::ptr::addr_of_mut!(**state)) }
        });

        let mut handle: *mut sys::keylt_lcd = std::ptr::null_mut();
        ...
        let handle = NonNull::new(handle).ok_or_else(|| {
            Error::local(
                Status::Internal,
                "keylt_lcd_open reported success but returned NULL",
            )
        })?;
    ...
    pub fn connect(&mut self) -> Result<()> {
        ...
        check(unsafe { sys::keylt_lcd_connect(self.handle.as_ptr()) })
    }
    ...
    pub fn is_connected(&self) -> Result<bool> {
        ...
        check(unsafe { sys::keylt_lcd_is_connected(self.handle.as_ptr(), &mut connected) })?;
        Ok(connected != 0)
    }
    ...
    pub fn upload_image(&mut self, image_path: impl AsRef<Path>) -> Result<()> {
        ...
        check(unsafe { sys::keylt_lcd_upload_image(self.handle.as_ptr(), path.as_ptr()) })
    }
}
...
impl Drop for Lcd {
    fn drop(&mut self) {
        ...
        // last callback.
        unsafe { sys::keylt_lcd_close(self.handle.as_ptr()) }
    }
}
