...
///
/// ```no_run
/// use keylt::{Keyboard, KeyState, Model};
///
...
/// ```
pub struct Keyboard {
    handle: NonNull<sys::keylt_keyboard>,
    model: Model,
}
...
impl Keyboard {
    ...
    pub fn open(model: Model) -> Result<Self> {
        let mut handle: *mut sys::keylt_keyboard = std::ptr::null_mut();
        // SAFETY: `handle` is a valid, writable out-parameter. On success the
        // library hands over ownership of a handle freed in `Drop`.
        check(unsafe { sys::keylt_keyboard_open(model.as_raw(), &mut handle) })?;

        let handle = NonNull::new(handle).ok_or_else(|| {
            Error::local(
                Status::Internal,
                "keylt_keyboard_open reported success but returned NULL",
            )
        })?;
    ...
    pub fn connect(&mut self) -> Result<()> {
        ...
        check(unsafe { sys::keylt_keyboard_connect(self.handle.as_ptr()) })
    }
    ...
    pub fn is_connected(&self) -> Result<bool> {
        ...
        check(unsafe { sys::keylt_keyboard_is_connected(self.handle.as_ptr(), &mut connected) })?;
        Ok(connected != 0)
    }
    ...
    pub fn info(&self) -> Result<KeyboardInfo> {
        ...
        check(unsafe { sys::keylt_keyboard_get_info(self.handle.as_ptr(), &mut raw) })?;
    ...
    pub fn set_active_key_ids(&mut self, key_ids: &[u8]) -> Result<()> {
        ...
        // call.
        check(unsafe {
            sys::keylt_keyboard_set_active_key_ids(
                self.handle.as_ptr(),
                key_ids.as_ptr(),
                key_ids.len(),
            )
    ...
    pub fn set_active_key_names<S: AsRef<str>>(&mut self, names: &[S]) -> Result<()> {
        ...
        check(unsafe {
            sys::keylt_keyboard_set_active_key_names(
                self.handle.as_ptr(),
                pointers.as_ptr(),
                pointers.len(),
            )
    ...
    pub fn active_key_count(&self) -> Result<usize> {
        ...
        check(unsafe {
            sys::keylt_keyboard_get_active_key_count(self.handle.as_ptr(), &mut count)
        })?;
        Ok(count)
    }
    ...
    pub fn set_keys(&mut self, state: KeyState) -> Result<()> {
        ...
        check(unsafe { sys::keylt_keyboard_set_keys(self.handle.as_ptr(), state.as_raw()) })
    }
    ...
    pub fn set_keys_capture(&mut self, state: KeyState, out: &mut [u8]) -> Result<usize> {
        ...
        let status = unsafe {
            sys::keylt_keyboard_set_keys_capture(
                self.handle.as_ptr(),
                state.as_raw(),
                out.as_mut_ptr(),
                out.len(),
    ...
    fn required_capture_len(&mut self, state: KeyState) -> Result<usize> {
        ...
        if status == sys::KEYLT_ERR_BUFFER_TOO_SMALL || status == sys::KEYLT_OK {
            Ok(written)
        } else {
    ...
    pub fn set_keys_rgb(&mut self, colour: impl Into<Rgb>) -> Result<()> {
        ...
        check(unsafe { sys::keylt_keyboard_set_keys_rgb(self.handle.as_ptr(), r, g, b) })
    }
    ...
    pub fn blink(&mut self, count: u32, interval: Duration) -> Result<()> {
        ...
        check(unsafe { sys::keylt_keyboard_blink(self.handle.as_ptr(), count, interval_ms) })
    }
}
...
impl Drop for Keyboard {
    fn drop(&mut self) {
        ...
        unsafe { sys::keylt_keyboard_close(self.handle.as_ptr()) }
    }
}
