...
//! keyboards.
//!
//! This wraps the `keylt` C ABI: opaque handles become RAII types, status codes
...
//! for info in keylt::list_available_keyboards()? {
...
//! # Linking
//!
//! The native library is located by [`keylt-sys`](keylt_sys), which by default
//! builds it from source with `CMake`. Set `KEYLT_LIB_DIR` and
//! `KEYLT_INCLUDE_DIR`, or install `keylt.pc` where pkg-config can see it, to
...
pub fn print_messages(buffer: &[u8], message_count: usize, message_length: usize) -> Result<()> {
    ...
    // will touch.
    error::check(unsafe {
        sys::keylt_print_messages(buffer.as_ptr(), message_count, message_length)
    })
}
