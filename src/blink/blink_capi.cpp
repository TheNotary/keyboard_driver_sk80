// Implementation of the portable C ABI declared in blink.h.
//
...
// -fvisibility=hidden; only the BLINK_API functions below are exported.
...
#ifndef BLINK_VERSION_STRING
#define BLINK_VERSION_STRING "0.0.0-unknown"
#endif
...
// error messages. The pointer handed out by blink_last_error_message() stays
// valid until the next blink_* call on the same thread.
std::string& LastError() {
    static thread_local std::string message;
    return message;
}

blink_status Fail(blink_status status, const char* what) noexcept {
    try {
...
blink_status Fail(blink_status status, const std::string& what) noexcept {
...
blink_status Guard(Fn&& fn) noexcept {
    LastError().clear();
    try {
        return fn();
    } catch (const std::invalid_argument& e) {
        return Fail(BLINK_ERR_INVALID_ARGUMENT, e.what());
    } catch (const std::out_of_range& e) {
        return Fail(BLINK_ERR_OUT_OF_RANGE, e.what());
    } catch (const std::logic_error& e) {
        // The library raises logic_error for protocol features a model has not
        // implemented yet.
        return Fail(BLINK_ERR_UNSUPPORTED, e.what());
    } catch (const std::bad_alloc&) {
        return Fail(BLINK_ERR_OUT_OF_MEMORY, "out of memory");
    } catch (const std::exception& e) {
        return Fail(BLINK_ERR_INTERNAL, e.what());
    } catch (...) {
        return Fail(BLINK_ERR_INTERNAL, "unknown error");
    }
}
...
// Deliberately a switch and not a cast: blink_key_value and blink::KeyValue
// have opposite orderings.
blink::KeyValue ToKeyValue(blink_key_value value) {
    switch (value) {
        case BLINK_KEY_ON:
            return blink::kOn;
        case BLINK_KEY_OFF:
    ...
    throw std::invalid_argument("unrecognised blink_key_value: " +
                                std::to_string(static_cast<int>(value)));
}

blink::KeyboardModel ToKeyboardModel(blink_model model) {
    switch (model) {
    ...
    throw std::invalid_argument("unrecognised blink_model: " +
                                std::to_string(static_cast<int>(model)));
}

int32_t FromKeyboardModel(blink::KeyboardModel model) {
    switch (model) {
        case blink::kSK80:
    ...
    throw std::invalid_argument("unmapped blink::KeyboardModel");
}

// Rejects the models that are named in the enum but have no driver behind them,
// so callers get BLINK_ERR_UNSUPPORTED instead of a vaguer failure from deeper
// in the library.
void RequireImplementedModel(blink_model model) {
    if (model == BLINK_MODEL_MK84) {
...
blink_keyboard_info ToCInfo(const blink::KeyboardInfo& info) {
    blink_keyboard_info out{};
    ...
    const size_t limit = BLINK_DISPLAY_NAME_CAPACITY - 1;
    size_t length = 0;
...
blink_status CopyInfos(const std::vector<blink::KeyboardInfo>& source,
                       blink_keyboard_info* out_infos, size_t capacity,
                       size_t* out_count) {
    ...
    *out_count = source.size();
    ...
    if (out_infos == nullptr) {
        ...
        return source.empty() ? BLINK_OK
                              : Fail(BLINK_ERR_BUFFER_TOO_SMALL, "buffer too small");
    }
    if (capacity < source.size()) {
        return Fail(BLINK_ERR_BUFFER_TOO_SMALL, "buffer too small");
    }
...
const KeyTable& KeyTableFor(blink_model model) {
...
// Bridges a caller-supplied blink_usb_io vtable onto the library's IUsbIO
// interface. The vtable is copied on construction, so the caller's struct need
// not outlive the handle.
class CApiUsbIO final : public blink::IUsbIO {
public:
    explicit CApiUsbIO(const blink_usb_io& io) : io_(io) {}
...
// Declared opaquely in blink.h; only this translation unit knows the layout.
struct blink_keyboard {
    blink_keyboard(blink_model model_in, blink::KeyboardModel cxx_model)
        : model(model_in), keyboard(cxx_model) {}

    blink_model model;
    blink::Keyboard keyboard;
};
...
struct blink_lcd {
    ...
    std::unique_ptr<blink::IUsbIO> io;
    std::string control_interface;
    std::string data_interface;
    blink::KeyboardLcd lcd;

    blink_lcd(std::unique_ptr<blink::IUsbIO> io_in, uint16_t vid, uint16_t pid,
...
BLINK_API const char* BLINK_CALL blink_status_name(int32_t status) {
    switch (status) {
        case BLINK_OK:
            return "BLINK_OK";
        case BLINK_ERR_INVALID_ARGUMENT:
            return "BLINK_ERR_INVALID_ARGUMENT";
        case BLINK_ERR_DEVICE_NOT_FOUND:
            return "BLINK_ERR_DEVICE_NOT_FOUND";
        case BLINK_ERR_OUT_OF_RANGE:
            return "BLINK_ERR_OUT_OF_RANGE";
        case BLINK_ERR_UNSUPPORTED:
            return "BLINK_ERR_UNSUPPORTED";
        case BLINK_ERR_IO:
            return "BLINK_ERR_IO";
        case BLINK_ERR_OUT_OF_MEMORY:
            return "BLINK_ERR_OUT_OF_MEMORY";
        case BLINK_ERR_BUFFER_TOO_SMALL:
            return "BLINK_ERR_BUFFER_TOO_SMALL";
        case BLINK_ERR_INTERNAL:
            return "BLINK_ERR_INTERNAL";
        default:
            return "BLINK_ERR_UNKNOWN";
    }
}
...
BLINK_API blink_status BLINK_CALL blink_list_known_keyboards(
    ...
    return Guard([&] { return CopyInfos(blink::known_keyboards, out_infos, capacity, out_count); });
}

BLINK_API blink_status BLINK_CALL blink_list_available_keyboards(
    blink_keyboard_info* out_infos, size_t capacity, size_t* out_count) {
    return Guard([&] {
        const std::vector<blink::KeyboardInfo> attached = blink::ListAvailableKeyboards();
...
BLINK_API blink_status BLINK_CALL blink_model_get_info(
    ...
    return Guard([&]() -> blink_status {
        if (out_info == nullptr) {
            throw std::invalid_argument("out_info must not be null");
        }
        RequireImplementedModel(model);
        const blink::KeyboardModel wanted = ToKeyboardModel(model);
        for (const auto& info : blink::known_keyboards) {
...
BLINK_API blink_status BLINK_CALL blink_keyboard_open(
    ...
    return Guard([&]() -> blink_status {
        ...
        *out_keyboard = nullptr;
        RequireImplementedModel(model);
        *out_keyboard = new blink_keyboard(model, ToKeyboardModel(model));
        return BLINK_OK;
    });
}

BLINK_API void BLINK_CALL blink_keyboard_close(blink_keyboard* keyboard) {
    // The sole destroy path. blink::Keyboard's destructor releases the device
...
BLINK_API blink_status BLINK_CALL blink_keyboard_connect(blink_keyboard* keyboard) {
    return Guard([&]() -> blink_status {
        ...
        if (!keyboard->keyboard.ConnectToDevice()) {
            return Fail(BLINK_ERR_DEVICE_NOT_FOUND, "no matching keyboard is attached");
        }
        return BLINK_OK;
    });
}
...
BLINK_API blink_status BLINK_CALL blink_keyboard_is_connected(
    ...
    return Guard([&]() -> blink_status {
        ...
        *out_connected = const_cast<blink::Keyboard&>(keyboard->keyboard).Found() ? 1 : 0;
        return BLINK_OK;
    });
}
...
BLINK_API blink_status BLINK_CALL blink_keyboard_get_info(
    ...
    return Guard([&]() -> blink_status {
        ...
        return blink_model_get_info(keyboard->model, out_info);
    });
}
...
BLINK_API blink_status BLINK_CALL blink_keyboard_set_active_key_ids(
    ...
    return Guard([&]() -> blink_status {
        ...
        const blink::AbstractKeyboard* spec = keyboard->keyboard.keyboard_spec;
        ...
        if (count >= static_cast<size_t>(blink::Keyboard::kMaxActiveKeys)) {
...
BLINK_API blink_status BLINK_CALL blink_keyboard_set_active_key_names(
    ...
    return Guard([&]() -> blink_status {
        ...
        return blink_keyboard_set_active_key_ids(keyboard, ids.data(), count);
    });
}
...
BLINK_API blink_status BLINK_CALL blink_keyboard_set_keys(
    ...
    return Guard([&]() -> blink_status {
        ...
        const blink::KeyValue key_value = ToKeyValue(value);
        if (!keyboard->keyboard.Found()) {
            return Fail(BLINK_ERR_DEVICE_NOT_FOUND, "keyboard is not connected");
        }
...
BLINK_API blink_status BLINK_CALL blink_keyboard_set_keys_capture(
    ...
    return Guard([&]() -> blink_status {
        ...
        if (capacity < required) {
            return Fail(BLINK_ERR_BUFFER_TOO_SMALL,
...
BLINK_API blink_status BLINK_CALL blink_lcd_open(
    ...
    return Guard([&]() -> blink_status {
        ...
        *out_lcd = nullptr;
        ...
        std::unique_ptr<blink::IUsbIO> transport;
        if (io == nullptr) {
            transport.reset(new blink::RealUsbIO());
        } else {
            transport.reset(new CApiUsbIO(*io));
        }

        *out_lcd = new blink_lcd(std::move(transport), vid, pid,
                                 std::string(control_interface),
                                 std::string(data_interface));
        return BLINK_OK;
    });
...
BLINK_API blink_status BLINK_CALL blink_lcd_connect(blink_lcd* lcd) {
    return Guard([&]() -> blink_status {
        ...
        if (!lcd->lcd.ConnectToDevice()) {
            return Fail(BLINK_ERR_DEVICE_NOT_FOUND, "no matching LCD device is attached");
        }
        return BLINK_OK;
    });
}
...
BLINK_API blink_status BLINK_CALL blink_lcd_is_connected(
    ...
    return Guard([&]() -> blink_status {
        ...
        *out_connected = const_cast<blink::KeyboardLcd&>(lcd->lcd).Found() ? 1 : 0;
        return BLINK_OK;
    });
}
...
BLINK_API blink_status BLINK_CALL blink_lcd_upload_image(
    ...
    return Guard([&]() -> blink_status {
        ...
        if (!lcd->lcd.UploadImage(std::string(image_path))) {
            return Fail(BLINK_ERR_IO, "the image upload did not complete");
        }
        return BLINK_OK;
    });
}
